...
import { Users, UserPlus, Phone, Banknote, Shield, Search, X, Loader2, ArrowUpRight, GraduationCap } from 'lucide-react';
...
const StaffManagement = ({ schoolId }) => {
    ...
    const [formData, setFormData] = useState({ firstName: '', lastName: '', phone: '', role: 'TEACHER', bankDetails: '', baseSalary: '' });
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await fetch(`${API_BASE}/payroll/staff`, {
                method: 'POST',
        ...
        <div className="space-y-8 animate-in fade-in duration-700">
            <header className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-500 font-medium mt-2">Manage educational enterprise personnel and salary structures</p>
                </div>
                <button onClick={() => setShowModal(true)} className="btn-primary shadow-2xl flex items-center gap-2 group">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="glass-card p-8 bg-white sophisticated-shadow border-slate-200">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Current Liabilities</p>
                    <h4 className="text-4xl font-black text-primary font-['Outfit']">₦{staff.reduce((acc, s) => acc + s.baseSalary, 0).toLocaleString()}</h4>
                </div>
                <div className="glass-card p-8 bg-white sophisticated-shadow border-slate-200">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Actionable Complaints</p>
                    <h4 className="text-4xl font-black text-rose-500 font-['Outfit']">{staff.reduce((acc, s) => acc + (s.complaints?.length || 0), 0)}</h4>
                </div>
            </div>

            <div className="glass-card overflow-hidden bg-white sophisticated-shadow border-slate-200">
                <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                    <h3 className="font-bold text-xl text-slate-900">Enterprise Personnel Ledger</h3>
                    <div className="relative">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                        <input type="text" className="bg-white border border-slate-200 rounded-xl pl-11 pr-6 py-3 text-xs font-medium w-64 outline-none focus:ring-2 focus:ring-primary/10 transition-all" placeholder="Search by name or role..." />
                    </div>
                </div>
                        ...
                        {staff.map((s) => (
                            <tr key={s.id} className="hover:bg-slate-50/50 transition-colors group">
                                <td className="px-8 py-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-0.5">Staff ID: {s.id}</p>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-8 py-6">
                                ...
                                <td className="px-8 py-6 text-right">
                                    <button className="p-3 rounded-xl hover:bg-white border border-transparent hover:border-slate-200 text-slate-400 hover:text-primary transition-all shadow-sm">
            ...
            {showModal && (
                <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-xl z-50 flex items-center justify-center p-6 animate-in fade-in duration-300">
                    <div className="glass-card w-full max-w-xl p-12 sophisticated-shadow bg-white animate-in zoom-in-95 duration-400 shadow-2xl relative">
                        ...
                        <div className="mb-10">
                            <h3 className="text-3xl font-black text-slate-900 mb-2">Personnel Induction</h3>
                            <p className="text-slate-500 text-sm font-medium">Initialize new staff entity with corporate parameters</p>
                        </div>
                        <form className="space-y-6" onSubmit={handleSubmit}>
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">First Identity</label>
                                    <input required className="input-field" placeholder="Human" onChange={e => setFormData({...formData, firstName: e.target.value})} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Last Identity</label>
                                    <input required className="input-field" placeholder="Resource" onChange={e => setFormData({...formData, lastName: e.target.value})} />
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Contact Vector (WhatsApp)</label>
                                    <input required className="input-field" placeholder="+234..." onChange={e => setFormData({...formData, phone: e.target.value})} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Commercial Grade (Salary)</label>
                                    <input required type="number" className="input-field font-['Outfit']" placeholder="50000" onChange={e => setFormData({...formData, baseSalary: e.target.value})} />
                                </div>
                            </div>
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Clearence Level (Role)</label>
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Commercial Ledger (Bank Info)</label>
                                <input className="input-field" placeholder="Bank Name - Account Number" onChange={e => setFormData({...formData, bankDetails: e.target.value})} />
                            </div>
                            <div className="flex gap-4 pt-4">
                                ...
                                <button type="submit" className="btn-primary flex-1 shadow-2xl">Confirm Induction</button>
                            </div>
                        </form>
                    </div>
                </div>
